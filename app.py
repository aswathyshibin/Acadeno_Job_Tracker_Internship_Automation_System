...
def send_email(jobs):
    ...
    # ✅ Subject
    subject = f"🌟Unlock Your Next Opportunity with Acadeno Technologies – {datetime.now().strftime('%d %b %Y')}"

    # ✅ Logo URL
    logo_url = "https://drive.google.com/uc?export=view&id=1wLdjI3WqmmeZcCbsX8aADhP53mRXthtB"

    # ✅ Warning if names & emails mismatch
    if len(student_names) != len(recipients):
        print(f"⚠️ Warning: {len(recipients)} emails but {len(student_names)} names — fallback will be used where needed.")

    for index, student_email in enumerate(recipients):
        # Match name or fallback to email prefix
        student_name = (
            student_names[index]
            if index < len(student_names) and student_names[index]
            else student_email.split("@")[0].replace(".", " ").replace("_", " ").title()
        )
        ...
        <p style="font-size:12px; color:#777; margin-top:35px; text-align:center;">
            Generated by Acadeno Technologies Pvt. Ltd. Job Tracker © 2025
        </p>
        </body>
        </html>
        """
